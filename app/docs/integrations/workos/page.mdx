<DocPage toc={[
  ...
  { title: "How This Fits the Gateway Pattern", url: "#how-this-fits-the-gateway-pattern", depth: 3 },
...
<Callout type="info">
  The live demo on convexbooking.dev uses **Convex Auth v2** (anonymous guest admin). This guide shows how to wire **WorkOS AuthKit** into the same gateway pattern instead.
</Callout>

This guide walks you through setting up WorkOS AuthKit as the identity provider for a ConvexBooking app of your own.

## Overview

[WorkOS AuthKit](https://workos.com/authkit) is one way to put a real identity provider behind the admin side of a booking app. The public sandbox at convexbooking.dev does not use it - it signs guest admins in with the Convex Auth v2 anonymous provider - but everything below plugs into the same place: the `adminQuery` / `adminMutation` gateway in `convex/functions.ts`. If you use WorkOS, this integration gives you a complete authentication system with minimal setup.

Booking itself is unaffected by the choice of provider: **taking a booking never requires a sign-in**. `publicQuery` / `publicMutation` stay anonymous, and authentication only guards the admin dashboard.
...
### How This Fits the Gateway Pattern

The app-side gate never talks to a provider SDK. It reads whatever identity Convex has validated on the request:
...
`adminQuery` / `adminMutation` wrap that in a `requireAuth()` helper which throws `ConvexError({ code: "UNAUTHENTICATED", ... })` when there is no identity, and injects `{ user, role: "admin" }` otherwise. `publicQuery` / `publicMutation` read no identity at all.

So swapping providers is a three-part change and nothing else:

1. Mount a different auth component in `convex/convex.config.ts` (WorkOS AuthKit instead of the Convex Auth v2 core).
2. Make the frontend send that provider's JWT to Convex.
3. Adjust the copy on the sign-in screen.

The booking gateway, the guards in `convex/public.ts` and every `adminQuery` call site stay byte-for-byte the same.
...
If you use WorkOS, AuthKit is mounted as a **sibling component** to ConvexBooking at the app level - the same slot where the live demo mounts the Convex Auth v2 core and its anonymous provider:
...
The shipped provider wraps the app in `ConvexAuthProvider` from `@convex-dev/auth/react`. To use WorkOS instead, replace it with an AuthKit bridge in `components/ConvexClientProvider.tsx`:
...
Link your admin shell to `/auth/signin?redirect=/admin` - that is the only place a sign-in belongs. The Booker never redirects there: booking is anonymous, and `createBooking` takes the booker's name and email as arguments instead of reading an identity.
...
2. In your app, open a page that calls an `adminQuery` (the dashboard)
...
### 6. Verify the Admin Gate

1. Sign out, then open `/admin` - the gate screen should ask you to sign in
2. Sign in with your test user - the dashboard loads and `adminQuery` calls resolve instead of throwing
3. Open `/book/studio-a` in a private window - booking still completes with no session at all

That last step is the important one: authentication gates the dashboard, never the booking flow.
...
- Every `adminQuery` / `adminMutation` throws `ConvexError({ code: "UNAUTHENTICATED" })`
