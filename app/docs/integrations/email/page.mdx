...
ConvexBooking automatically sends booking confirmation and cancellation emails when configured with Resend. This integration is completely optional—the system works perfectly without it.
...
├─ auth            Convex Auth v2 core — guest-admin sessions
├─ authAnonymous   "Continue as guest admin" login provider
└─ booking         @mrfinch/booking
   └─ resend       nested component — transactional email
```

Only `booking` matters for email: Resend lives *inside* it. This creates "componentception"—a component within a component, each with isolated tables. Emails are automatically triggered by booking lifecycle events (`booking.created`, `booking.cancelled`).

<Callout type="info">
  The two `auth` components are what this demo happens to mount for its admin dashboard; email works exactly the same in an app that mounts only `booking`.
</Callout>
...
```typescript
// APP LAYER (convex/public.ts) — can read process.env
...
export const createBooking = publicMutation({
  handler: async (ctx, args) => {
    return await ctx.runMutation(components.booking.public.createBooking, {
      ...
      // sanitized copies built earlier in the handler
      booker,
      location,
...
`baseUrl` is optional — it is `NEXT_PUBLIC_APP_URL` and only decides which origin the manage/cancel links in the email point at.

```typescript
// COMPONENT LAYER (@mrfinch/booking — component/public.ts)
...
- `convex/public.ts` → `createBooking`, `cancelBookingByToken`, `rescheduleBookingByToken` (anonymous bookings — all three call the shared `resendOptions()` helper)
- `convex/admin.ts` → `transitionBookingState`, `confirmBooking`, `declineBooking`, `cancelReservation`, `rescheduleBooking`, `cancelMultiResourceBooking` (dashboard operations)
...
1. Open the manage/cancel link from the confirmation email (or find the booking in the admin dashboard)
...
**Solution:**
Check booking data structure in Convex Dashboard → Data → `bookings` table
...
1. ✅ Environment variables set in Convex
2. ✅ `resendOptions` passed in gateway functions
3. ✅ Domain verified in Resend (or using test domain)
4. ✅ API key valid and has "Send Emails" permission
5. ✅ Convex logs show "Triggering hooks" message
6. ✅ No errors in Resend dashboard → **Logs**
7. ✅ Check spam folder
8. ✅ DNS records propagated (use [dns checker](https://dnschecker.org))
...
```bash
# In Convex Dashboard
# Settings → Environment Variables → Delete RESEND_API_KEY
```

**Option 2: Comment Out in Code**
```typescript
// convex/public.ts — one helper feeds every public mutation
function resendOptions() {
  return undefined; // emails disabled
